...
def build_intraday_prompt(
    *,
    ...
    lines: List[str] = [
        ...
        "## 指令",
        "",
        "1. 综合昨日盘后分析阈值和今日盘中实时走势，给出每支监控股票的买卖建议，按买入/卖出止盈/卖出止损/观望分组。",
        ...
        "10. 最终建议区禁止使用Markdown表格，必须按买入/卖出建议分组展示。",
        "11. 每只股票只能出现在一个分组中；异常放量作为该股票条目的补充说明，不单独成组。",
        ...
        "## 决策分析过程",
        "",
        "基于规则，对每只触发了事件的股票进行逐一分析：",
        "",
        "### 1. 买入信号相关股票",
        '- **示例股票（000001）**：当前价...，进入...买入区间；量比...；大盘...；相对强弱...；建议...。',
        "",
        "### 2. 卖出信号相关股票（止盈）",
        '- **示例股票（000002）**：当前价...，已突破止盈位...；量比...；大盘...；建议卖出止盈。',
        "",
        "### 3. 卖出信号相关股票（止损）",
        '- **示例股票（000003）**：当前价...，跌破止损位...；量比...；建议卖出止损。',
        "",
        "### 4. 观望/无法判断股票（如有）",
        '- **示例股票（000004）**：当前价...，未触及明确买卖阈值/数据不足/信号矛盾，建议观望。',
        "",
        "---",
        "",
    ]
